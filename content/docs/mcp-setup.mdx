...
Pons exposes a [Model Context Protocol](https://modelcontextprotocol.io/) server via Streamable HTTP transport. If your MCP client supports OAuth, use dynamic client registration first. API keys are still supported as the fallback path for clients that only accept a static bearer token.

## 1. Recommended: OAuth Dynamic Client Registration
